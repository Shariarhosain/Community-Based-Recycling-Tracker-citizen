...
export default function addContribution() {
  ...
  const [imageFile, setImageFile] = useState(null); // Store the image file locally
  const [imageName, setImageName] = useState(''); // Store the unique image name
  const [activeItem, setActiveItem] = useState("addContribution");
  ...
  const generateUniqueImageName = () => {
    return `image_${Date.now()}.jpg`; // Example: `image_1619510224245.jpg`
  };
  const handleSubmit = async (e) => {
    ...
    if (
      !formData.recyclingCenterName.trim() ||
    ...
    try {
      // Construct the data to send (without the image file itself)
      const contributionData = {
        recyclingCenterName: formData.recyclingCenterName.trim(),
        material_type: formData.material_type.trim(),
        quantity: Number(formData.quantity),
        message: formData.message.trim(),
        picture: imageName, // Send the unique image name, not the image file
      };
  
      console.log("Contribution Data: ", contributionData); // Log contribution data for verification
  
      // If there's an image, upload it to the server and save it
      if (imageFile) {
        const formDataForUpload = new FormData();
        formDataForUpload.append('image', imageFile, imageName); // Append the image file with the unique name
       
  
        // Upload the image to the backend
        const imgResponse = await axios.post(
          'http://localhost:3001/citizen/upload',
          formDataForUpload,
          {
            headers: {
      ...
      // Now send the form data (including the image name) to the backend for contribution
      const response = await axios.post(
        'http://localhost:3001/citizen/contribute',
        contributionData,
        {
          headers: {
      ...
      if (response.status === 201) {
        ...
        // Reset form data and image state after successful submission
        setFormData({
          recyclingCenterName: '',
          material_type: '',
          quantity: '',
  ...
  const handleImageChange = (e) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        setImagePreview(reader.result); // Display preview of the image
      };
      reader.readAsDataURL(file); // Generate preview URL
  
      setImageFile(file); // Save the file locally
      const uniqueName = generateUniqueImageName(); // Generate unique name for the image
      setImageName(uniqueName); // Set the unique name
    }
  };
  ...
  useEffect(() => {
    ...
    axios
      ...
      .catch((error) => {
        ...
        Cookies.remove("access_token"); // ✅ Remove expired/invalid token
        router.replace("/?unauthorized=true");
      })
  ...
  const handleItemClick = (itemName: string) => {
    ...
    // ✅ Ensure nothing is displayed while loading
    if (loading) {
      return (
...
<div className="p-4 mt-[80px] sm:ml-64">
  <div className="flex items-center justify-center min-h-screen bg-gray-100">
    <form onSubmit={handleSubmit} className="bg-white p-8 rounded-lg shadow-xl max-w-lg w-full">
      <h1 className="text-3xl font-semibold mb-6 text-center text-gray-800">Contribute Your Recycling</h1>

      {/* Recycling Center Name */}
      <div className="mb-6">
        <label htmlFor="recyclingCenterName" className="block mb-2 text-lg font-medium text-gray-700">Recycling Center Name</label>
        <input
          type="text"
          id="recyclingCenterName"
          name="recyclingCenterName"
          value={formData.recyclingCenterName}
          onChange={(e) => setFormData({ ...formData, recyclingCenterName: e.target.value })}
          className="w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#92DA9E] text-gray-800"
          required
        />
      </div>
      ...
      {/* Material Type */}
      <div className="mb-6">
        <label htmlFor="material_type" className="block mb-2 text-lg font-medium text-gray-700">Material Type</label>
        <input
          type="text"
          id="material_type"
          name="material_type"
      ...
      {/* Image File */}
      <div className="mb-6">
        <label htmlFor="picture" className="block mb-2 text-lg font-medium text-gray-700">Upload Image</label>
        <input
          type="file"
          id="picture"
          name="picture"
      ...
      <button
        type="submit"
        disabled={loading}
        className="w-full py-3 bg-[#92DA9E] text-black font-bold rounded-lg shadow-md hover:bg-[#76C79C] transition-colors duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
      >
