...
export default function addContribution() {
  ...
  const [activeItem, setActiveItem] = useState("recyclingCenters");
  ...
  useEffect(() => {
    const fetchRecyclingCenters = async () => {
      ...
      try {
        ...
        const response = await axios.get('http://localhost:3001/citizen/search-center', {
          headers: {
  ...
  }, [searchTerm, recyclingCenters]);

  const handleSearchChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setSearchTerm(e.target.value); 
  };

  const renderRecyclingCenterCards = () => {
    return filteredCenters.map((center, index) => (
      <div key={center.center_name} className="p-4 mt-[80px] sm:ml-64 w-full md:w-1/2 lg:w-1/2 xl:w-1/3">
        <div className="max-w-sm bg-white border border-gray-200 rounded-lg shadow-sm dark:bg-gray-800 dark:border-gray-700">
          <a href={center.link_to_place_marked} target="_blank" rel="noopener noreferrer">
            <img
              className="rounded-t-lg object-cover w-full h-48"
              src="https://thumbs.dreamstime.com/b/cold-stream-multi-coloured-grasses-8799136.jpg" // Example random image for each card
          ...
          <div className="p-5">
            ...
            <p className="mb-3 font-normal text-gray-700 dark:text-gray-400">{center.location}</p>
            <p className="mb-3 font-normal text-gray-700 dark:text-gray-400">Contact: {center.contact_number}</p>
            <p className="mb-3 font-normal text-gray-700 dark:text-gray-400">Note: {center.Note}</p>
            <a
              href={center.link_to_place_marked}
              target="_blank"
...
<div className="bg-gray-100">
          ...
          {/* Search Bar */}
          <div className="flex justify-center p-4 mt-12 add">
        <input
          type="text"
          placeholder="Search by Location (e.g., Mirpur)"
          value={searchTerm}
      ...
      {/* Display the Recycling Centers */}
      <div className="flex flex-wrap justify-center gap-4">
        {filteredCenters.length > 0 ? (
          renderRecyclingCenterCards()
        ) : (
          <div className="w-full text-center">No recycling centers found.</div>
        )}
      </div>
